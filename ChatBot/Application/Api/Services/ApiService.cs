...
namespace Application.Api.Services
{
    public abstract class ApiService : IApiService
    {
        ...
        public async Task<byte[]?> DownloadFile(ApiSettings apiSettings, object body)
        {
            ...
            SetQueryParams(request, apiSettings.QueryParams.QueryParams);

            var rawResponse = await apiSettings.RestClient.DownloadDataAsync(request);
        ...
        public virtual void SetQueryParams(RestRequest request, List<DataSettingsBase> queryParams)
        {
            if (!queryParams.Any())
                return;

            queryParams.ForEach(p => request.AddQueryParameter(p.Key, p.Value));
        }
